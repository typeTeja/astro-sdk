...
class HorizonService:
    ...
    def calculate_event(
        ...
        """
        jd = date.julian_day
        res_jd = self.eph.calculate_rise_set(
            jd, planet, lat, lon, altitude, is_rise=is_rise, rsmi_extra=rsmi_extra
        )
        return Time.from_julian_day(res_jd) if res_jd else None

    def calculate_sunrise(
        self, date: Time, lat: float, lon: float, altitude: float = 0.0
    ) -> Time | None:
        """Calculate sunrise."""
    ...
    def calculate_transit(
        self, planet: Planet, date: Time, lat: float, lon: float, altitude: float = 0.0
    ) -> Time | None:
    ...
    def calculate_twilight(
        ...
        if twilight_type not in types:
            raise ValueError(
                f"Unknown twilight type: {twilight_type}. Must be one of {list(types.keys())}"
            )

        bit = types[twilight_type]

        # Dawn (rise) and Dusk (set)
        dawn = self.calculate_event(
            Planet.SUN, date, lat, lon, altitude, is_rise=True, rsmi_extra=bit
        )
        dusk = self.calculate_event(
            Planet.SUN, date, lat, lon, altitude, is_rise=False, rsmi_extra=bit
        )
