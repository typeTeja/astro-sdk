...
from ..domain.cycle import CycleConfig, CycleEvent
...
class CycleService:
    ...
    def compute_return(
        self, planet: Planet, target_long: float, start_time: Time, window_days: float = 400.0
    ) -> CycleEvent:
        """
    ...
    def _refine_return(
        self, planet: Planet, target_lon: float, jd1: float, jd2: float, tolerance: float = 1e-7
    ) -> float:
        low = jd1
        high = jd2
        for _ in range(30):
