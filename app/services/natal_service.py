from ..core.constants import HouseSystem, Planet, SiderealMode
from ..core.ephemeris import Ephemeris
from ..core.time import Time
from ..domain.house import ChartHouses, HouseAxes, HouseCusp
        ...
        for planet in Planet:
            ...
            if lat is not None and lon is not None:
                hor = self.eph.calculate_horizontal(
                    jd, data["longitude"], data["latitude"], data["distance"], lat, lon, alt
                )
                azimuth = hor["azimuth"]
            ...
            if planet == Planet.MEAN_NODE:
                ...
                k_az: float | None = None
                k_alt: float | None = None
                if lat is not None and lon is not None:
                    k_hor = self.eph.calculate_horizontal(
                        jd, ketu_lon, -data["latitude"], data["distance"], lat, lon, alt
                    )
                    k_az = k_hor["azimuth"]
